...
function Tabella(){
  ...
  const [PersonaDaModificare, SetPersonaDaModificare] = useState('');
  
 
  function elimina(s : String): void{
  ...
  function inserisci(){
    ...
    for (let i = 0; i < person.length; i++) {
      if (person[i].name == PersonaDaModificare) {
        person[i].name = nome
        person[i].gender = genere
        person[i].age = parseInt(anno)
        break;
      }
    }
    setPerson(person)
